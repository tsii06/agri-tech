...
const IntrantsDisplay = ({
  ...
  const formatPeriodeApplication = () => {
    ...
    const dateDebut = dateRecoltePrecedente
      ? new Date(dateRecoltePrecedente).toLocaleDateString("fr-FR", {
          day: "2-digit",
          month: "short",
        })
      : "Début parcelle";
    const dateFin = new Date(dateRecolte).toLocaleDateString("fr-FR", {
      day: "2-digit",
      month: "short",
    });
    ...
    <div className="mb-2">
      ...
      <div className="mt-1">
        {intrants.slice(0, maxVisible).map((intrant, index) => {
          ...
          const conformeRegle = validateIntrantForHarvest(
            intrant,
            dateRecolte,
            dateRecoltePrecedente
          );
          ...
          } - Ajouté le: ${intrant.dateAjout || "Date inconnue"}${
            intrant.quantite ? ` - Quantité: ${intrant.quantite}` : ""
          }${intrant.unite ? ` ${intrant.unite}` : ""}${
            intrant.description ? ` - ${intrant.description}` : ""
          }${
            dateRecolte
              ? conformeRegle
                ? " ✓ Conforme à la règle"
                : " ⚠️ Hors période réglementaire"
              : ""
          }`;
        ...
        {intrants.length > maxVisible && (
          <span
            ...
            title={`Autres intrants: ${intrants
              .slice(maxVisible)
              .map((i) => i.nom || i.type || "Intrant")
              .join(", ")}`}
            style={{ cursor: "help" }}
          >
